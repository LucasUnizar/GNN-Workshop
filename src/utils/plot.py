...
def plot_prediction_and_target(z_net, z_gt, X, Y, save_dir='outputs/gifs/', name='Liver_actuator'):
    """
    ...
    points_net = np.c_[X.ravel(), Y.ravel(), z_net[0, :].ravel()]  # Initial Z values (first time step)
    points_gt = np.c_[X.ravel(), Y.ravel(), z_gt[0, :].ravel()]  # Initial Z values for ground truth

    # Create a PyVista plotter with subplots
    plotter = pv.Plotter(shape=(1, 2), notebook=False, off_screen=False)

    # Open a GIF file to save the animation
    plotter.open_gif(save_dir / f'{name}_scatter_comparison.gif')

    # Animation settings
    nframe = z_gt.shape[0]  # The number of frames (time steps)
    min_val, max_val = z_gt.min(), z_gt.max()

    for i in range(nframe):
        # Clear points for the next frame
        plotter.clear()
        # Update z_net and z_gt for the current time step
        z_net_frame = z_net[i, :]  # Predicted values
        z_gt_frame = z_gt[i, :]  # Ground truth values

        # Update Z-coordinates
        points_net[:, 2] = z_net_frame.ravel()
        points_gt[:, 2] = z_gt_frame.ravel()

        # Plot z_net in the left subplot
        plotter.subplot(0, 0)
        plotter.add_points(
            points_net,
            scalars=z_net_frame.ravel(),
            cmap="viridis",
        ...
        plotter.add_text("Prediction", position="top", font_size=12, color="black", shadow=True,)

        # Plot z_gt in the right subplot
        plotter.subplot(0, 1)
        plotter.add_points(
            points_gt,
            scalars=z_gt_frame.ravel(),
            cmap="viridis",
            render_points_as_spheres=True,
            point_size=7,
            clim=[min_val, max_val],
        ...
        plotter.add_text("Ground Truth", position="top", font_size=12, color="black", shadow=True,)
        plotter.add_scalar_bar(
            title="u",
            vertical=False,
            position_x=0.25,  # Centered horizontally under the subplot
            position_y=0.05,  # Adjust height to place below the plot
            width=0.5,  # Adjust width to fit the subplot
        )
    ...
    print(f"Scatter animation saved as {save_dir / f'{name}_scatter_comparison.gif'}")
...
def plot_3D(z_net, z_gt, X, Y, prediction=True, save_dir='outputs/gifs/', name='Liver_actuator', wandb_flag=False):
    """
    This function creates a 3D scatter plot comparing Data Driven MeshGraphs predictions with ground truth
    and saves the data as .mat files for later use in MATLAB.
    ...
    # Assume X, Y, z_net, z_gt, save_dir, and name are predefined
    # Flatten the data for scatter plot
    points = np.c_[X.ravel(), Y.ravel(), z_net[0, :].ravel()]  # Initial Z values (first time step)

    # Create a PyVista plotter with subplots
    plotter = pv.Plotter(notebook=False, off_screen=True)

    # Open a GIF file to save the animation
    plotter.open_gif(save_dir + f'/{name}_{"pred" if prediction else "target"}_scatter.gif')
    ...
    for i in range(nframe):
        # Update z_net and z_gt for the current time step
        if prediction:
            z_frame = z_net[i, :]  # Predicted values
            # Update Z-coordinates
            points[:, 2] = z_frame.ravel()
            plotter.add_text("Prediction", position="top", font_size=12, color="black", shadow=True, )
        else:
            z_frame = z_gt[i, :]  # Ground truth values
            points[:, 2] = z_frame.ravel()
            plotter.add_text("Ground truth", position="top", font_size=12, color="black", shadow=True, )
        ...
        plotter.add_points(
            points,
            scalars=z_frame.ravel(),
            cmap="plasma",  # Color map
            render_points_as_spheres=True,
            point_size=7,
            clim=[min_val, max_val],  # Adjust scalar color limits
            show_scalar_bar=False
        )
        plotter.add_scalar_bar(
            title="u",
        ...
        # Write a frame for the current Z update
        plotter.write_frame()
        # Clear the previous points to prepare for the next frame
        plotter.clear()
    ...
    if wandb_flag:
        wandb.log({f'{name}_{"pred" if prediction else "target"}_scatter': wandb.Image(save_dir + f'{name}_{"pred" if prediction else "target"}_scatter.gif')})

    print(f"Scatter animation saved as {save_dir + f'/{name}_scatter.gif'}")
