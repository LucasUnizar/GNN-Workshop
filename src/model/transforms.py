...
class FaceToEdgeTethra(BaseTransform):
    ...
    def forward(self, data: Data) -> Data:
        ...
        # if hasattr(data, 'face_res'):
        #
        #     face_res = data.face_res.transpose(1, 0)
        #     edge_low_res_index = torch.cat([face_res[0:2], face_res[1:3], face_res[2::], face_res[0::2], face_res[1::2], face_res[0::3]], dim=1)
        #     edge_low_res_index = to_undirected(edge_low_res_index, num_nodes=data.x.shape[0])
        #
        #     data.edge_low_res_index = edge_low_res_index
        #     if self.remove_faces:
        #         data.face_res = None
...
class RadiusGraphMesh(BaseTransform):
    ...
    def forward(self, data: Data) -> Data:
        ...
        edge_contact_index = torch_geometric.nn.radius_graph(
            data.pos,
            self.r,
            batch,
            self.loop,
        ...
        edge_contact_index = edge_contact_index[:, mask]
        src, dest = edge_contact_index
        ...
        edge_contact_index = edge_contact_index[:, keep_index]
        src, dest = edge_contact_index
        ...
        data.edge_contact_index = edge_contact_index
...
class ContactDistance(BaseTransform):
    ...
    def forward(self, data: Data) -> Data:
        (src, dest), pos = data.edge_contact_index, data.pos
...
@functional_transform('add_steady_step')
class AddSteadyStep(BaseTransform):
    r""" Custom class to compute mesh distances.
